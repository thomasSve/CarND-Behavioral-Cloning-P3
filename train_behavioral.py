...
def train_model(model, X_train, X_validation, y_train, y_validation, batch_size = 32, epochs = 100, learning_reate = 0.001):
    ...
    model.fit_generator(batch_generator(X_train, y_train, batch_size = batch_size), samples_per_epoch=None, nb_epoch=epochs, verbose=1, callbacks=[early_stopping_monitor], validation_data=batch_generator(X_validation, y_validation, batch_size))
